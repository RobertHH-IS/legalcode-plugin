...
**Current MCP contract.** Legalcode advertises exactly five tools:

| Operation | Current tool |
|---|---|
| Discover jurisdictions, fields, facets, values, and help | `legalcode_discover` |
| Search every supported source family | `legalcode_search` |
| Fetch metadata, excerpts, sections, or full-text pages | `legalcode_fetch` |
| Trace indexed relationships | `legalcode_trace` |
| Run supported aggregate analysis | `legalcode_analyze` |

Search accepts one `jurisdiction` string and a `sourceTypes` array. Never invent source-specific tool names or filters.
...
|---|---|---|
| In-force IS act (consolidated) | `sources/act/lög-NN-YYYY-current.md` | `legalcode_search sourceTypes:["law"] jurisdiction:"IS" mode:"exact" filters:[{lawKey eq "NN/YYYY"}]`. The default returns the in-force version (`versionRole: "current"`). Do not pass `language`. |
| EU directive/regulation | `sources/directive/{CELEX}-en.md` | `legalcode_search sourceTypes:["law"] jurisdiction:"EU" mode:"exact" filters:[{lawKey eq "<CELEX>"}]`. CELEX (e.g. `32016R0679`) is the canonical lookup. |
...
| ESA decisions | `sources/esa/{ref}.md` | `legalcode_search sourceTypes:["guidance"] jurisdiction:"IS"` (or `jurisdiction:"EU"`) with agency/ESA terms. |
...
| Implementing reglugerðir | `sources/reglugerðir/{NN-YYYY}.md` | `legalcode_search sourceTypes:["law"] jurisdiction:"IS" filters:[{actType eq "REGULATION"}, {lawKey eq "<NN/YYYY>"} for known numbers]`. |
